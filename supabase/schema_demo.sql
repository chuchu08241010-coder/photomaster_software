...
-- ============================================================
-- DEMO 专用追加段（网友体验版数据库）
-- 说明：本文件 = 完整 schema + 下面的 demo 种子数据，整份粘贴运行一次即可。
-- ★务必另外手动：Authentication → Sign In/Providers → 打开 "Anonymous Sign-Ins" → Save
--   （体验版用匿名身份 + 邀请码登录，不发邮件）
-- ============================================================

-- 生成 50 个永久邀请码：PM-DEMO-001 .. PM-DEMO-050（永久有效、可反复使用、不消费）
insert into public.invite_codes(code, permanent)
select 'PM-DEMO-' || lpad(g::text, 3, '0'), true
from generate_series(1, 50) g
on conflict (code) do update set permanent = true;

-- 查看全部永久码：
--   select code from public.invite_codes where permanent order by code;
